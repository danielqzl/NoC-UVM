...
class noc_base_seq extends uvm_sequence#(noc_transaction);
    ...
    virtual task body();
        ...
        repeat (total_packets) begin    
            pkt_vc = (vc != 2'b00) ? vc : def_vc_list[$urandom_range(0, 1)]; 
            pkt = noc_transaction::type_id::create("pkt");
            assert(pkt.randomize() with {
                source == client_id; // force source to agent ID
                dest_map < (1 << num_nodes);  
                dest_map != 7'b0;    // must send to at least one client
                vc == pkt_vc;
                data != 0;
             })
            else 
