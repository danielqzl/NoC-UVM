...
#(
    ...
    logic [0:VC_CNT-1][0:N-1] voq_inhibit_d1 [N];
    // logic inhibit_line [N];
            ...
            for (genvar k = 0; k < N; k++) begin  // per physcial output port 
                ...
                if (i != k) begin
                    noc_voq #(
                        ...
                        .enqueue(voq_enq[i][j]),
                        .flit_in(flit_in[i]),
                        // .inhibit_in(inhibit_line[i]),
                        .inhibit(voq_inhibit[i][j][k]), 
                        .inhibit_d1(voq_inhibit_d1[i][j][k]), 
                        .grant(voq_grant[l][i]),
                    ...
                    assign voq_inhibit_d1[i][j][k] = 1'b0;
                end
            end
        end
    end
    ...
    always_comb begin : u_enq_ctrl
        for (int i = 0; i < N; i++) begin
            ...
            for (int j = 0; j < VC_CNT; j++)
                voq_enq[i][j] = ((voq_inhibit_d1[i][j] == '0) && (link_i[i].valid == 1'b1));  
            // back-pressure
